...
Deploy `QuietShield_License_Server_v1.2.6_Code372_Chrome.gs`, supplied with the R7 full-functional bundle. Code372 itself is unchanged by the R7 DNR client fix.
...
The existing server requires `adminPublicKey` when an administrator key is activated. R7 generates a 2048-bit RSA device keypair locally and sends only the public SPKI material. The administrator activation key is never bundled and is removed from Chrome storage after a successful admin activation.
...
- `ADMIN_DEVICE_KEY_REQUIRED`: an older Chrome client is being used; R7 supplies the required device public key.
